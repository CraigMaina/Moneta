...
- `200 { status: 'manual', raw: string }` — parseable:false / any validation failure / any Gemini-call failure. Client should prefill manual entry with `raw`.
- `401 unauthorized` (missing/invalid JWT) · `400 invalid_request` (bad body) · `405 method_not_allowed` · `500 server_error` (missing `SUPABASE_URL`/`SUPABASE_ANON_KEY`/`GEMINI_API_KEY` secrets) — all `{ error: { code, message } }`, never a stack trace or the API key.

**Provider/model:** Google Gemini `gemini-2.0-flash` (free tier; replaced Anthropic per user directive). Structured output via `generationConfig.responseSchema`; envelope `{ parseable, transaction }`; strict zod validation (`interpretModelJson`) is the only trusted gate.

**Deploy + secrets (for the lead/user to run — this environment has no Supabase login/Docker):**
```bash
supabase functions deploy parse-sms
supabase secrets set GEMINI_API_KEY=...   # free key: https://aistudio.google.com/apikey
```
...
**Assumptions recorded in DECISIONS.md:** the (functionally-no-op-today) partial-vs-plain unique index choice on `raw_sms_hash`; `resolved` has no automatic-flip mechanism yet; no service-role key used anywhere in this function; Gemini structured-output (responseSchema) with strict zod validation as the only trusted gate; a Gemini infra failure folds into the same `manual` 200 response as a validation failure (never a 5xx the client can't act on); the `deno.json` import-map trick that makes `schema.ts`/`prompt.ts` importable unmodified by both Deno and Vitest; the new `eslint.config.js` Deno-globals override; `rls.test.sql`'s extension.
...
- Edge Function deploy + secret: `supabase functions deploy parse-sms`; `supabase secrets set GEMINI_API_KEY=... (free: https://aistudio.google.com/apikey)`. `parse_misses` migration + `npm run test:rls` need running against the cloud DB.
